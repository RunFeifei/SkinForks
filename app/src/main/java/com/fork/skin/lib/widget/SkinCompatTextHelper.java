...
public class SkinCompatTextHelper extends SkinCompatHelper {
    ...
    private void applyTextSizeResource() {
        ...
        if (mTextSizeResId == R.dimen.abc_text_size_body_1_material
                || mTextSizeResId == R.dimen.abc_text_size_caption_material) {
            return;
        }
        if (mTextSizeResId != INVALID_ID) {
            float size  = SkinCompatResources.getInstance().getDimension(mTextSizeResId);
